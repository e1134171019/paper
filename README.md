...
> Current 2026-08-20 exploratory override: Ryan/R2 deep-dive is paused as comparator-only work. The topology-synthesis mainline is restored to the nine-family → X1/X2/X3 → PM-combination → actual-graph workflow. `R7 = PM1 + PM3 + PM7` remains the active branch, but File 50 rejected the obvious cascaded HFT+SC and HFT+voltage-doubler graphs. File 51 then showed that lower HFT secondary voltage does not automatically reduce re-optimized transformer copper/window burden at fixed 2 kW. `k=2 / 180 V` and `k=3 / 120 V` are retained only for a matched transformer-geometry crossover; `k>=4` is deprioritized. No R7 half-cycle charge/stack graph is authorized for PSIM until real magnetic Rac/leakage/volume savings can exceed the added PM3 charge/RMS loss.
