...
> Current 2026-08-20 mainline: File 63 searched the accessible R52 design/Drive source set for the real X3 PWM carrier and did **not** recover a defensible carrier scalar, so X3 switching watts remain open. It did, however, convert the physical R52 BUS from unconstrained `OPEN` to a first-principles/component-rating bound: a two-level bridge producing 220-Vrms requires `VBUS >= 244.36 V`, while the verified four parallel `680 µF / 315 V` capacitors across the full bus impose a present steady design ceiling of `315 V`; therefore the A0-real 220-V BUS is `244.36–315 V / BOUNDED / NOT MEASURED`. This implies an ideal post-X1 current scale of `6.35–8.18 A` and a universal T1/T2 secondary RMS lower bound `>=6.35 A`, while actual capacitor-input charging RMS remains higher/open. The exact R52 transformer P/N and DCR/Rac were not linked; other `M1-PQ50-*` records remain `CONTEXT_ONLY`. The passive 2720-µF X2 bank sees an ideal 2ω current scale of roughly `4.49–5.79 Arms` total, and the matching KMR 315-V/680-µF catalog class is about `2.10 Arms/cap @105°C/120 Hz`, so E5 is not established as a large watt-loss bucket. Consequently `E4+E6` remains a conditional comparator/modernization branch rather than a primary new-topology branch. Immediate next is File64: a minimum A0 measurement pack for BUS, X3 carrier/dead time, T1/T2 secondary RMS, and de-energized winding DCR. Candidate #10 remains `HOLD / NOT_ASSIGNED`; novelty remains `NOT_ESTABLISHED`; PSIM/LTspice/hardware remain not executed.
...
- [`research/63_A0_X3_SWITCHING_AND_HFT_COPPER_RMS_CLOSURE_V1.md`](research/63_A0_X3_SWITCHING_AND_HFT_COPPER_RMS_CLOSURE_V1.md) — exhausts the current source search for X3 carrier, bounds the physical 220-V R52 BUS at `244.36–315 V`, derives post-X1/HFT secondary-current lower bounds, checks passive-X2 2ω current against KMR ripple-current context, and moves the mainline to a minimum physical-measurement pack rather than another topology graph.
