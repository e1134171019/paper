...
> Current 2026-08-20 mainline: File 61 closed the first thermal/dynamic/X2/HFT uncertainty envelope around the File-60 `E4+E6` post-HFT double-processing crossover. The nominal 25°C direct-HF-link matrix reference still survives conduction, but the preferred baseline-low/candidate-hot gate is much tighter: a 40-mΩ SiC class leaves only about `2–4 W` robust conduction margin once hot `RDS(on)` is admitted, while a 60-mΩ class can fail conduction before matrix switching or X2 interaction are counted. File61 also corrects the File60 transformer-current proxy: a PDM direct-HF-link main differential current is about `7.9 Arms` at `Vs≈350 V`, so buffer/common-mode interaction is not fixed at 50%; for `Vbuf≈200–350 V`, first-order `κ²` spans roughly `0.26–0.80`. Therefore `E4+E6 = NOMINAL CONDITIONAL_SURVIVOR / ROBUST WIN NOT_ESTABLISHED / PRIOR-ART-RICH`, and it is downgraded from the primary topology-synthesis target. No new generic graph is authorized. Immediate next is File62: recover/close the actual A0 X3 semiconductor and switching frequency, rectifier hot-VF contract, HFT RMS/DCR/Rac/copper-loss range, HV-link ESR/ripple loss, and the matched reduced-order A0-vs-G13 comparator contract. Candidate #10 remains `HOLD / NOT_ASSIGNED`; novelty remains `NOT_ESTABLISHED`; PSIM/LTspice/hardware are not executed.
