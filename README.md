...
> Current 2026-08-20 mainline: File 60 executed the matched-loss crossover for the joint `E4+E6` post-HFT double-processing boundary (`secondary rectifier → HV-link/X2 → VSI`) against the known direct-HF-link matrix/cycloconverter reference. Using a conservative 650-V SiC matched contract, the standalone rectifier floor is ~17.14 W and a practical four-die matrix path beats the baseline rectifier+two-die-VSI **on conduction alone** whenever matched MOSFET `RDS(on) < ~104 mΩ`; current 15–60-mΩ classes therefore survive this first gate. However the remaining conduction headroom is only ~7–15 W, so matrix switching/commutation, center-tap X2 buffer current, transformer RMS increment and Lbuf/Cbuf loss can readily reverse the result. A simple `κ≈0.707` buffer/main-current sensitivity implies ~50% extra transformer-copper exposure, making the actual A0 transformer burden a first-order discriminator. Therefore `E4+E6 = CONDUCTION SURVIVOR / TOTAL-LOSS CROSSOVER_UNRESOLVED`, not a new candidate. The known Takaoka/Itoh and Blinov HF-link soft-switching families are hard comparators. Immediate next is File 61: close the A0 VSI/rectifier/HFT and merged-matrix/X2 uncertainty envelope before any new graph is synthesized. Candidate #10 remains `HOLD / NOT_ASSIGNED`; novelty remains `NOT_ESTABLISHED`; PSIM/LTspice/hardware are not executed.
