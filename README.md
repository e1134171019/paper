...
## 2026-09-18 integration-scope convergence and first-circuit plan

[Integration-scope convergence and first-circuit validation plan](research/WORKING_BRANCH_2026-09-18_INTEGRATION_SCOPE_CONVERGENCE_AND_FIRST_CIRCUIT_PLAN_V1.md) fixes the current exploratory synthesis branch after the lineage/Delta/trade-off review. The preferred research host is a center-tapped self-oscillating push-pull / Royer-derived power stage, but it is not declared optimal. The first research cell now uses selective/partial integration: keep main HF transfer, self-oscillating feedback, explicit Lm/Llk/Coss interaction and a legal transfer→commutation→transfer path inside the cell; allow small Lcomm/Cr/clamp/reset and supervisory correction outside if the loss ledger requires them. Reconfigurable turns, secondary stacking, flying-cap gain sharing, SST multi-cell functions, direct-HF-link AC synthesis and active 2ω decoupling are deferred to layer 2. The next artifact must be a node-level first schematic with qT1→qK12→qT2→qK21 states and mandatory PSIM/oscilloscope probes. Novelty remains NOT_ESTABLISHED; PSIM/HW remain NOT_EXECUTED; Candidate #10 remains HOLD / NOT_ASSIGNED. This does not supersede the authoritative A0 physical-measurement mainline.
