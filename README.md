...
> Current 2026-08-20 exploratory override: File 54 executed the first `O23 = X1 separate, X2+X3 shared post-X1` actual graphs. `G23-A1` used split/dynamic DC-link capacitors as both the 2ω energy store and the three-level/T-type/NPC AC-synthesis rails; `G23-A2` screened the analogous integrated flying-capacitor power-pulsation-buffer mechanism. Both are physically valid, but multi-route Gate A found established prior art for T-type/NPC integrated active power decoupling, dynamic three-level DC-link swing, split-capacitor decoupling, and flying-capacitor integrated PPB. Therefore `G23-A1/A2 = STOP_AS_NOVELTY / REFERENCE`, PSIM is not authorized for either as a proposed topology. A retained first-principles result is the X2/X3 interaction bound `Vdc,min >= Vout,pk`: at a 350-V center rail, only ~11.1% ideal downward swing is available before 220-Vac peak synthesis is lost, so capacitance reduction by larger 2ω voltage swing must be paid for with higher average Vdc/device stress. Immediate next is `G23-B = differential-storage AC synthesis` with exact graph + immediate Gate A. R2/Ryan remains comparator-only; R7 is weak/deferred; Candidate #10 remains HOLD and novelty NOT_ESTABLISHED.
